...
def main():
    ...
    for rho in rho_center_space:
        ...
        while lrho[-1] > precisao:
            ...
            # O valor da densidade ficou negativo, logo, excluı́mos todos os últimos valores calculados nesse raio especı́fico, e diminuı́mos o h pela metade
            lrho.pop()
            lm.pop()
            lr.pop()
            lhp.pop()
...
def plot_graph(lr_estrela, lm_estrela, lc, lk2, p_center_space, rho_center_space):
    ...
    # Obtem a curva k2 esperada do arquivo .dat
    c_esperado, k2_esperado = np.loadtxt(fname="k2_vs_c_polytropic_n_1.dat", usecols=(0, 1), unpack=True)

    # Faz o plot das curvas k2 vs C
    plt.figure()
    plt.plot(array_c, array_k2, linewidth=1, label="Curva calculada", marker='.')
    plt.plot(c_esperado, k2_esperado, linewidth=1, label="Curva esperada", marker='.')
    plt.plot(0, 0.2599, color='k', label="Valor Newtoniano", marker='o')
    ...
    plt.title("Curva k2 vs Densidade central $[m^{-2}]$")
    plt.xlabel("Densidade central")
    plt.ylabel("k2 [adimensional]")
    plt.legend()
    plt.show()
